...
## AI context: sp-managed vs. project-owned

`docs/ai-context/` has two lanes, so a repo's own context never collides with what `sp`
populates:

- **sp-managed** — `index.md`, `overview.md`, `rules.md`, `github-workflow.md`. Generated by
  `sp init` and refreshed by `sp init --update`. **Do not hand-edit these**; changes are
  either overwritten on update or silently forfeit future updates.
- **project-owned** — `project.md`. Created once by `sp init` and **never overwritten**, even
  on `--update`. Put this repo's project-specific AI context here — domain facts, local
  conventions, data sources, gotchas. `index.md` links it so AI assistants read it.

`CLAUDE.md` follows the same split: `sp` owns a delimited `<!-- BEGIN/END llmflow-init -->`
block; everything outside it is yours.
