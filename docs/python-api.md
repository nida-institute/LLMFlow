...
- **If you can point to it in the YAML, it's an attribute** (`p.output_file_directory`,
  ...
  `p.run()`).
- Attribute names come straight from the pipeline schema (`PIPELINE_SCHEMA`) — `Step`'s
  attributes are generated from it, so they cannot drift. A syntax key that is a Python
  keyword gets a trailing underscore (`for:` → `step.for_`, `in:` → `step.in_`), and a hyphen
  becomes an underscore (`group_by:` → `step.group_by`). No invented names.
- `Step` is flat and generic: it exposes the union of every step type's keys, and a key that
  does not apply to a given step reads as `None` (`step.size` on an `llm` step). Which keys are
  *valid* on a type is per-type and enforced by the linter — `allowed_step_keys(step_type)`
  returns that set, or `None` for a plugin/registered type whose keys cannot be enumerated.
...
- **`PIPELINE_SCHEMA`** — the JSON Schema of the pipeline syntax; it enumerates every
  attribute the object model exposes (`llmflow.PIPELINE_SCHEMA`). The step schema is
  type-discriminated (common `properties` plus `allOf` `if type == …` branches), so it
  describes the real grammar rather than a flat union. Read it through the derived helpers
  rather than walking it yourself:

```python
from llmflow.pipeline_schema import allowed_step_keys, common_step_keys, step_keys

allowed_step_keys("llm")        # keys valid on an llm step
allowed_step_keys("xpath")      # None — plugin type, keys cannot be enumerated
common_step_keys()              # keys valid on any type
step_keys()                     # the union; the object model's attribute set
```
...
The `llmflow` namespace is the contract; the object model's attributes are generated from
`PIPELINE_SCHEMA` and guarded by a drift test, and every method delegates to the engine's own
