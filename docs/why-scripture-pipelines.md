...
from GitHub repositories (CSV/TSV, XML, JSON — the formats most open biblical datasets
use), run a Python function, query XML databases (BaseX/XQuery) or relational databases
(DuckDB), apply XSLT transformations, or read Paratext projects (USFM/USJ). The plugin
system is designed for extensibility — you can write a plugin for any data source or
tool you need. Steps pass their output forward; every intermediate result is
...
- Are people engaging the text, or just approving AI output?
