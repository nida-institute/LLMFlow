...
- **Linting** — the linter's allowed-key sets are derived from `PIPELINE_SCHEMA`
  (`ALLOWED_STEP_KEYS = _allowed_step_keys_from_schema()` in `utils/linter.py`), so a key the
  schema doesn't define is flagged and a key it *does* define is never a false positive.
- **The public object model** — `Pipeline` / `Step` (`model.py`) expose one attribute per
  schema key (the `${...}` you can point to in the YAML).
- **Testing** — a schema-*derived* drift test (`tests/test_pipeline_model.py`) reads
  `PIPELINE_SCHEMA` at runtime and asserts the object model's attribute set matches it in both
  directions — no missing keys, no invented names. The schema is the test's oracle, not a
  hard-coded list, so the test is what makes "the API tracks the language" enforced rather than
  aspirational. (The test is *derived* from the schema; we don't code-generate test files.)
...
and the API move with it. Nothing re-encodes the vocabulary a second time.
...
- **`llmflow.PIPELINE_SCHEMA`** — the *nouns*: every syntax key, as JSON Schema. Attribute
  accessors on the object model map to these one-to-one (§16), with a total naming rule (leaf
  key; a Python keyword gets a trailing underscore, e.g. `for:` → `step.for_`).
