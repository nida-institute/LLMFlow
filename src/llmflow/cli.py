...
#: `list_pipelines` is defined below rather than imported from `cli_utils`. The import was here
#: too, and the definition shadowed it, so the imported one never ran.
...
def build_parser():
    ...
    clean_p.add_argument(
        "--debug-only",
        action="store_true",
        help="Delete only debug files (intermediate_file_directory/debug/ or outputs/debug/)",
    )
    clean_p.add_argument(
        "--intermediate-only",
        action="store_true",
        help="Delete only intermediate files, preserving debug/",
    )
    clean_p.add_argument(
        "--var",
        action="append",
        default=[],
        help="Pipeline variable key=value; repeatable (honored when resolving directories)",
    )
    ...
    init_p.add_argument(
        "--update",
        action="store_true",
        help="Regenerate files that carry the '<!-- Generated by llmflow init -->' marker, "
        "leaving hand-edited files untouched",
    )
    init_p.add_argument(
        "--sync",
        action="store_true",
    ...
    ldb_p.add_argument(
        ...
        action="store_true",
        dest="list_drivers",
        help="List available database drivers",
    )
...
def main(argv=None):
    ...
    if args.command == "context":
        from llmflow.context import format_context_list, list_context_files
    ...
    if args.command == "content":
        ...
        if args.content_command == "status":
            from llmflow.utils.content_status import format_status, get_content_status
            ...
            from llmflow.utils.content_list import format_content_list, list_content
