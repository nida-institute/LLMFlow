...
**This is not the map of *your* project.** That is `project-index.md`, which is yours to write
and which `sp` never overwrites — put your own entry documents, design notes and references
there, or in documents it points to.
...
#: Canonical engine documents, which live in the engine repository rather than in a project.
#: They are not catalogued paths — nothing writes them into a project — so they are listed
#: here rather than derived. Kept deliberately short: anything a project *receives* belongs in
#: the catalog above, where it cannot go stale.
ENGINE_REFERENCES = """
## Engine reference, canonical

- [Pipeline language spec](https://github.com/nida-institute/LLMFlow/blob/main/docs/llmflow-language.md)
- [Tutorial](https://github.com/nida-institute/LLMFlow/blob/main/docs/tutorial.md)
