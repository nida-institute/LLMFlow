# GitHub Authority Policy — All Scripture Pipeline Projects

These rules apply to every AI session on this machine, across all registered pipeline projects.
...
**Recording which account is yours, not the engine's.** Put the machine user's address and GitHub username in `~/.sp/user-context/` — that directory is never shipped and `sp init` never overwrites it. Point `env.GH_CONFIG_DIR` in your Claude Code settings at that account's `gh` configuration so AI sessions authenticate as it rather than as you.
