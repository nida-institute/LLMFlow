...
- `docs/ai-context/` — AI-facing documentation (read `project-index.md` first)
...
**HARD PROHIBITION: Never modify the sp-generated files under `docs/ai-context/`** (`sp-index.md`, `overview.md`, `rules.md`, `github-workflow.md`). `project-index.md` is **not** in that list — it is the project's own map, created once and never overwritten.
...
**The rules for this project are in `docs/ai-context/rules.md`. Read it first — it is
...
def __getattr__(name: str):
    ...
    `tools/update_ai_context.py`. Both wrote `docs/ai-context/rules.md`, so which rules a
...
read and keep current. The generated files in `docs/ai-context/` (`sp-index.md`, `overview.md`,
`rules.md`, `github-workflow.md`) are refreshed by `sp init --update` — do not hand-edit them;
anything you want to keep and evolve locally goes here or in `project-index.md`, which is also
...
**Read this first.** It is *your* map, not Scripture Pipelines' — `sp` wrote it once and will
never overwrite it, so add to it freely.

## Start here, every session

- `project/TODO.md` — active work, what is in flight, what not to touch
- `docs/ai-context/rules.md` — the rules this project is held to. Authoritative
- `docs/ai-context/project.md` — this project's own facts, conventions and gotchas

## What Scripture Pipelines ships

`docs/ai-context/sp-index.md` lists every document `sp` writes and what each is for — the
pipeline YAML reference, the tutorial, the audit checklists. It is rendered from the file
catalog, so it is always current. Do not hand-edit it.
...
them, a field reference, data notes. Anything a session must read before working here belongs
in this list, or in `project.md`, or in a document one of them points to.
...
def init_project(base_dir: Path, update: bool = False, no_examples: bool = False) -> None:
    ...
    sp_index_path = ai_context_dir / "sp-index.md"
