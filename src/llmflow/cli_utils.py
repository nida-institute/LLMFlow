...
- Use `size`/`stride` when list items are independent (summarise every 10 reviews); use a
  ...
  logical units" in the LLMFlow language reference for why.
...
authoritative.** Start from `docs/ai-context/index.md`, which maps every topic to the
file that governs it.
...
def _install_claude_skills(base_dir: Path, sp_home: Optional[Path] = None) -> list[str]:
    ...
    `<repo>/.claude/skills/` (project). `~/.sp/skills/` is LLMFlow's own directory and is
