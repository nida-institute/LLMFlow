...
three disciplines went unshipped for months because a hand-kept list disagreed with what the
...
def test_shared_discipline_carries_no_engine_vocabulary(discipline: str):
    ...
    Where the removed text was an engine rule, it belongs in an engine-only discipline —
    `sp-workflow.md` for the CLI and for per-pipeline tracking. Nothing is deleted; it moves
    to the file that owns it.
    """
    path = _disciplines_dir() / discipline
    assert path.is_file(), f"{discipline} is not shipped"

    offenders = _offenders(path.read_text(encoding="utf-8"), ENGINE_VOCABULARY)

    assert not offenders, (
        f"{discipline} is shared with Human at the Helm but names this engine:\n  "
        + "\n  ".join(offenders)
    )
...
def test_shared_discipline_serves_both_toolchains(discipline: str):
    ...
    """
    text = (_disciplines_dir() / discipline).read_text(encoding="utf-8")
    ...
    assert not missing, (
        f"{discipline} shows {', '.join(sorted(present))} tooling but not "
        f"{', '.join(sorted(missing))}. Name the counterpart command beside it "
        f"(pytest ↔ vitest, hatch ↔ npm/pnpm, ruff ↔ tsc)."
    )


def test_every_shipped_discipline_is_classified():
    """No discipline may be added without deciding whether Human at the Helm gets it.

    An unclassified discipline is one nobody has asked the governing question about, and it
...
@pytest.mark.parametrize("discipline", ENGINE_ONLY)
def test_engine_only_discipline_keeps_its_specificity(discipline: str):
    ...
    """
    path = _disciplines_dir() / discipline
    assert path.is_file(), f"{discipline} is no longer shipped"
    assert _offenders(path.read_text(encoding="utf-8"), ENGINE_VOCABULARY), (
        f"{discipline} no longer names this engine — if it was generalized, §4 needs "
...
def test_the_split_kept_the_prohibition_on_running_pipelines():
    ...
    """
    text = (_disciplines_dir() / "sp-workflow.md").read_text(encoding="utf-8")
...
def test_the_split_left_no_engine_content_behind_in_the_general_half():
    ...
    """
    text = (_disciplines_dir() / "workflow.md").read_text(encoding="utf-8")
